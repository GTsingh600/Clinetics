...
import { ApiError, NetworkError, apiFetch, type User } from "@/lib/api";
...
export function LoginForm({ next }: { next?: string }) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      // A NetworkError means the request never arrived, which is a different
      // problem from bad credentials and needs a different message.
      setError(
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-card border border-outline-variant bg-container-lowest p-6 shadow-sm"
    >
      ...
      <label className="mt-4 block text-sm font-medium text-secondary" htmlFor="password">
        Password
      </label>
      <input
        id="password"
      ...
      {error ? (
        <p role="alert" className="mt-4 rounded-card bg-danger/10 px-3 py-2 text-sm text-danger">
          {error}
        </p>
      ) : null}
      ...
      <button
        type="submit"
        disabled={busy}
        className="mt-6 w-full rounded-card bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-opacity hover:opacity-90 disabled:opacity-50"
      >
